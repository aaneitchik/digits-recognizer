...
import algo from './AlgorithmService';
...
const Algorithm = reactStamp(React).compose({
	state: {},
	generateDigit() {
		const generatedDigit = algorithm.generateDigit(this.state.selectedDigit.value);
		this.setState({
			generatedDigit
		});
		const context = document.getElementById('digit-canvas').getContext('2d');
		mnist.draw(generatedDigit, context);
	},
	recognizeDigit() {
		const result = algorithm.recognize(this.state.generatedDigit);
		...
		return (
			<div>
				<table className="result-table">
					<thead><tr>{indices}</tr></thead>
					<tbody><tr>{closeness}</tr></tbody>
				</table>
	...
	render() {
		return (
			<div className="algorithm-content">
				<div>Choose digit:</div>
				<div className="select-container">
					<Select name="digit"
							value={this.state.selectedDigit}
							options={digits}
							onChange={this.onSelectChange.bind(this)}/>
				</div>
				<div>
					<button className="btn"
							onClick={this.generateDigit.bind(this)}>Generate
					</button>
				</div>
